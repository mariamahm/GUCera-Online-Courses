...
print 'Promocode created successfully'
...
print 'Promocode issued successfully'
...
create proc FindCourseID
@name varchar(40),
@cid int output
as
select @cid = id
from course
where name = @name and (accepted = 0 or accepted is null)
...
as
declare @cid int 
exec FindCourseID @name = @course, @cid = @cid output
exec AdminAcceptRejectCourse @adminid = @adminID, @courseId = @cid

go
create proc CourseInfo
@name varchar(10)
as begin 
select id,creditHours,name,price,content,adminId,insid from Course inner join InstructorTeachCourse on Course.id=InstructorTeachCourse.cid
